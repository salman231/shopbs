...
function updateproduct($adminToken,$Baseurl,$productallData){
	...
	$tierPriceArr = array();	
	if(is_null($productallData['prdGroupPrice']))
	{
		$tierPricesFinalArr = array();
	}
	else
	...
	if($update_array['prdImage'] == 1){
		...
		if(sizeof($galleryArr) > 0 && count($prd_imge) > 0){
	...
	//***************Get product image URL Start**********
	
	$hostImageURL = "";
	$prdFullURL = "";
	if(sizeof($data) > 0){
		$imgGallery = $data['media_gallery_entries'][0];
		if(sizeof($imgGallery) > 0){
			$MainURL = str_replace("index.php/","",$Baseurl);
			$hostImageURL = $MainURL.'pub/media/catalog/product'.$imgGallery['file'];
		}
		$custAttr = $data['custom_attributes'];
		if(sizeof($custAttr) > 0){
			//$MainURL = str_replace("index.php/","",$Baseurl);
			foreach($custAttr as $addAtt){
				$addAtt = (object)($addAtt);
				$addAttCode = $addAtt->attribute_code;
				$AttValue = $addAtt->value;				
				if($addAttCode == 'url_key')
				{					
					$prdFullURL = $AttValue;
				}
			}
			//$prdFullURL = $MainURL.$custAttr['file'];
		}
	}
	//return $data;
	if($prd_id !=''){
		$returnArr = array("status" =>'1', "product_id"=>$prd_id,"product_url"=>$prdFullURL,"hosted_image_url"=>$hostImageURL,"image_Posted_url"=>$url, "message"=>'Product updated successfully', 'new_ceated_imgs'=>$imagename,'NewImage'=>$imagename);
		return $returnArr;
	}
	else{
...
function createproduct($adminToken,$product_url,$productallData,$Baseurl){
	...
	if($prd_id !=''){
		...
		$returnArr = array("status" =>'1', "product_id"=>$prd_id,"product_url"=>$prdFullURL,"hosted_image_url"=>$hostImageURL,"image_Posted_url"=>$url, "message"=>'Product created successfully', 'new_ceated_imgs'=>$imagename,'NewImage'=>$imagename);
		return $returnArr;
...
function XXXXXXXXXXUpdateAttributesForCatelogProduct_OLD($prdData,$adminToken,$Baseurl){
   ...
   $sku = $prdData['sku']; 			
	foreach($prdData['additionalAttributes'] as $addAtts){
			...
			foreach($addAtts as $addAtt){
										...
										$addAtt = (object)($addAtt); 
										$addAttCode = $addAtt->key;
										$addtributeID = getAttributeDetailsByCode($addAttCode,$Baseurl,$adminToken);
										//$addAttCode. " ".$addtributeID."<br/>";
										if($addtributeID <= 0){
												...
												$attData = array(
												   ...
												   "frontend_labels" => array(array("store_id" => "0", "label" => $addAtt->key)),
												 //  "options"=> $options,
												...
												$response = curl_exec($ch);
      											...
      											assingAttributesetBycode($addAttCode,$Baseurl,$adminToken);
...
function assingAttributesetBycode($addAttCode,$Baseurl,$adminToken)
{
	$attData = array(
		"attributeSetId"  =>4,
		"attributeGroupId"=>7,
		"attributeCode"   =>$addAttCode,
		"sortOrder"       => 100,

	);
	$att_post_url = $Baseurl. "rest/V1/products/attribute-sets/attributes";
	$attData      = json_encode($attData);
