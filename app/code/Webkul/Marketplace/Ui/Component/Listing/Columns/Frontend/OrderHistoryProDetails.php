    ...
    public function prepareDataSource(array $dataSource)
    {
        ...
        if (isset($dataSource['data']['items'])) {
            ...
            foreach ($dataSource['data']['items'] as &$item) {
                ...
                $vendorTaxAmount = 0;
                $itmorder = $objectManager->create('Magento\Sales\Model\ResourceModel\Order\Tax\Item')->getTaxItemsByOrderId($item['order_id']);
                $taxpercent = 0;
                foreach ($itmorder as $key => $taxval) {
                    if (isset($taxval['tax_percent'])) {
                        $taxpercent = $taxval['tax_percent'];
                        break;
                    }
                }
                if ($taxToSeller) {
                    if($taxpercent == 0){
                        ...
                        $selleractamt = $item['actual_seller_amount']*($taxpercent/100);
                        $vendorTaxAmount = $selleractamt;    
                    }
                }
