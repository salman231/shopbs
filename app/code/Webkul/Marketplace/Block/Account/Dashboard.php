...
class Dashboard extends \Magento\Framework\View\Element\Template
{
	...
	foreach ($collection as $item) {
		$totalTax += $item->getData('total_tax');
	}
    ...
    public function lifetimesales(){
        ...
        $collection = $this->mpSaleslistCollectionFactory->create();
        $collection->addFieldToFilter('seller_id', ['eq' => $sellerId]);
        $collection->addFieldToFilter('cpprostatus', ['eq' => '1']);
        $sum = 0;
        foreach($collection as $item){
            ...
            $commission = $this->_mphelper->getConfigCommissionRate();
            $quantity = $item['magequantity']; 
            $mainPrice = ($productPrice - ($productPrice * $commission/100 ));
            $finalPrice = 0;
            
            if($item['total_tax'] <= 0 && $registerwithtax == '0'){
                $finalPrice = $quantity * ($mainPrice - (($mainPrice * $item['total_tax']) /100));
            }elseif($item['total_tax'] > 0 && $registerwithtax == '1'){
    ...
    public function pendingAmountWithTax(){
        $sellerId = $this->getCustomerId();
        $collection = $this->mpSaleslistCollectionFactory->create();
        $collection->addFieldToFilter('seller_id', ['eq' => $sellerId]);
        $collection->addFieldToFilter('paid_status', ['eq' => '0']);
        $sum = 0;
        foreach($collection as $item){
            $registerwithtax = $item['seller_vat'];
    ...
    public function totalPayout(){
        ...
        $collection = $this->mpSaleslistCollectionFactory->create();
        $collection->addFieldToFilter('seller_id', ['eq' => $sellerId]);
        $collection->addFieldToFilter('paid_status', ['eq' => '1']);
        $collection->addFieldToFilter('cpprostatus', ['eq' => '1']);
        $sum = 0;
        foreach($collection as $item){
            $registerwithtax = $item['seller_vat'];
