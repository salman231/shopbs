...
class MassPayseller extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        try {
            ...
            $salesReceivedAmount = 0;
            $TotalSalesAmount = 0;
            $salesReceivedAmounrinv = 0;
            $actparterprocost = 0;
            $totalamount = 0;
            ...
            //Ravi code start
            $totalSalesCollection = $this->collectionFactory->create()
            ->addFieldToFilter('seller_id', $sellerId);
            
            foreach($totalSalesCollection as $totalSales){
                $registerwithtax = $totalSales['seller_vat'];
                $productPrice = $totalSales['magepro_price'];
                $commission =  $this->mpHelper->getConfigCommissionRate();
                $mainPrice = ($productPrice - ($productPrice * $commission/100 ));
                $old_qty = $totalSales['magequantity'];
                if($totalSales['total_tax'] <= 0 && $registerwithtax == '0'){
                    $TotalSalesAmount += $old_qty * ($mainPrice - (($mainPrice * $totalSales['total_tax']) /100));
                }elseif($totalSales['total_tax'] > 0 && $registerwithtax == '1'){
                    $TotalSalesAmount += $totalSales['actual_seller_amount'];
                }else{
                    $TotalSalesAmount += $old_qty * ($productPrice - (($productPrice * $commission) /100));
                }
            }
            //Ravi code end
            ...
            foreach ($collection as $row) {
                ...
                if (count($collectionverifyread) >= 1) {
                    ...
                    $registerwithtax = $row['seller_vat'];
                    $productPrice = $row['magepro_price'];
                    $commission = $this->mpHelper->getConfigCommissionRate();
                    $quantity = $row['magequantity'];
                    $mainPrice = ($productPrice - ($productPrice * $commission/100 ));
                    if($row['total_tax'] <= 0 && $registerwithtax == '0'){
                        $salesReceivedAmount = $quantity * ($mainPrice - (($mainPrice * $row['total_tax']) /100));
                    }elseif($row['total_tax'] > 0 && $registerwithtax == '1'){
                        $salesReceivedAmount = $row['actual_seller_amount'];
                    }else{
                        $salesReceivedAmount = $quantity * ($productPrice - (($productPrice * $commission) /100));
                    }
                    
                    $totalrecived = $salesReceivedAmount + $amountpaid;
                    $totalremain = $TotalSalesAmount - $totalrecived;
                    $verifyrow = $this->saleperpartner->create()->load($id);
                    $verifyrow->setTotalSale($TotalSalesAmount);
                    $verifyrow->setLastAmountPaid($salesReceivedAmount);
                    ...
                    $commission = $this->mpHelper->getConfigCommissionRate();
                    $quantity = $row['magequantity'];
                    $commission_rate =  ($productPrice * $commission/100 );
                    $mainPrice = ($productPrice - ($productPrice * $commission/100 ));
                    ...
                    $totalremain = $TotalSalesAmount - $salesReceivedAmount;
                    
                    $percent = $helper->getConfigCommissionRate();
                    $collectionf = $this->saleperpartner->create();
                    $collectionf->setSellerId($sellerId);
                    $collectionf->setTotalSale($TotalSalesAmount);
                    $collectionf->setLastAmountPaid($salesReceivedAmount);
                    $collectionf->setAmountReceived($salesReceivedAmount);
                    $collectionf->setAmountRemain($totalremain);
                    $collectionf->setCommissionRate($percent);
                    $collectionf->setTotalCommission($commission_rate);
                ...
                $orderinfo = $orderinfo."<tr>
                    ...
                    <td class='item-price'>".$order->formatBasePrice($salesReceivedAmount).'</td>
                </tr>';
                $salesReceivedAmounrinv += $salesReceivedAmount;
            }
            ...
            if ($actparterprocost) {
                ...
                $emailTempVariables['myvar4'] = $order->formatBasePrice($salesReceivedAmounrinv);
