    ...
    public function orderPlacedOperations($order, $lastOrderId)
    {
        ...
        foreach ($sellerOrder as $info) {
            ...
            foreach ($fetchsale as $res) {
                ...
                $productImage = $_imageHelper->init($product, 'small_image', ['type'=>'small_image'])->keepAspectRatio(true)->resize('65','65')->getUrl();
                $image = '<img src="'.$productImage.'" alt="'.$productName.'">';

                $sku = $product->getSku();
                $orderinfo = $orderinfo."<tbody><tr>
                                            <td class='item-price'>".$image."</td>
                                            <td class='item-info'>".$productName."</td>
                                            <td class='item-info'>SKU: ".$sku."</td>
            ...
            $orderinfo = $orderinfo."<tfoot class='order-totals'>
                                        <tr class='subtotal'>
                                            <th colspan='3'>".__('Total Items')." = </th>
                                            <td colspan='3'><span>".$totalQty."</span></td>
                                        </tr>
                                    </tfoot>";
            ...
            $emailTempVariables['seller_name'] = $receiverInfo['name'] . ' ' .$seller_name;
        ...
        */
        ...
        foreach ($order->getAllItems() as $item) {
            ...
            if ($itemData['product_type'] != 'bundle') {
                ...
                // $newcommison = ($actparterprocost * 12) / 100;
                $actparterprocost = $actparterprocost + ($actparterprocost * 12) / 100;
...
/* start validate supplier has added VAT/TAX number in profile if yes or Move Product Tax to Supplier Account then set TotalTax else set to zero */
            $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
            $customerData = $objectManager->create('Magento\Customer\Model\Customer')->load($sellerId);
            //$vatStatus = $customerData['registerwithvat'];
            $vattaxno = $customerData['taxvat'];
            if(!empty($vattaxno) || $taxToSeller){
                $collectionsave->setTotalTax($taxamount);
            }
            else{
                $collectionsave->setTotalTax(0);
            }
            /* logic end here*/
            ...
            // if($commission != 0){
            //     $commission = $newcommison + $commission;
            // } 
