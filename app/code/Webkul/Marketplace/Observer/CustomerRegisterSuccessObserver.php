        ...
        try {
            ...
            if (!empty($paramData['is_seller']) && !empty($paramData['profileurl']) && $paramData['is_seller'] == 1) {
                ...
                if (!$profileurlcount->getSize()) {
                    ...
                    //echo "<pre>";print_r($paramData);exit;
                    ...
                    $model->save();
                    //Register with vat data code start
                    $connection  = $this->resource->getConnection();
                    $data = ["registerwithvat"=>$paramData['registerwithvat']]; // Key_Value Pair
                    $id = $customerid;
                    $where = ['entity_id = ?' => (int)$id];
             
                    $tableName = $connection->getTableName("customer_entity");
                    $connection->update($tableName, $data, $where);
                    //Register with vat data code end   
