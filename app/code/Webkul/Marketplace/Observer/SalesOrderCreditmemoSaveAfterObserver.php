...
class SalesOrderCreditmemoSaveAfterObserver implements ObserverInterface
{
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        ...
        $taxToSellerr = $helper->getConfigTaxManage();
...
/*logic start validate supplier has added VAT/TAX number in profile if yes or Move Product Tax to Supplier Account then set TotalTax else set to zero */
                if($sellerProduct['seller_id']){
                    $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
                    $customerData = $objectManager->create('Magento\Customer\Model\Customer')->load($sellerProduct['seller_id']);
                    //$vatStatus = $customerData['registerwithvat'];
                    $vattaxno = $customerData['taxvat'];
                    if(!empty($vattaxno) || $taxToSellerr){
                        $sellerProduct->setTotalTax($remainTaxAmount);
                    }
                    else{
                        $sellerProduct->setTotalTax(0);
                    }
                }
/*logic ends here*/
                //$sellerProduct->setTotalTax($remainTaxAmount);
