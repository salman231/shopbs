...
class SalesOrderSuccessObserver implements ObserverInterface
{
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        ...
        foreach ($orderIds as $lastOrderId) {
            ...
            // $data = ["seller_vat"=>$vatStatus]; // Key_Value Pair
            // $id = $lastOrderId;
            // $where = ['order_id = ?' => (int)$id];
     
            // $tableName = $connection->getTableName("marketplace_saleslist");
            // $connection->update($tableName, $data, $where);
            //Ravi code end
            $selllist = $this->saleslistFactory->create()
            ->getCollection()
            ->addFieldToFilter('order_id', $lastOrderId);
            foreach ($selllist as $slist) {
