...
namespace Vmj\Custom\Controller\Account;
        ...
        if ($this->getRequest()->isPost()) {
            try {
                ...
                if (empty($errors)) {
                    ...
                    /*if ($fields['registerwithvat']!='') {
                        $objectManager = \Magento\Framework\App\ObjectManager::getInstance();   
                        $resourceConnection = $objectManager->get('\Magento\Framework\App\ResourceConnection');  
                        $connection  = $resourceConnection->getConnection(); 
                         $tableName = $this->getTableName('customer_entity');
                        
                        $sql = "UPDATE $tableName SET `registerwithvat` = '".$fields['registerwithvat']."'  WHERE $tableName.`entity_id` = ".$sellerId;
        ...
        foreach ($fields as $code => $value) {
            switch ($code):
                ...
                case 'taxvat':
                    ...
                    break;
                     case 'registerwithvat':
                    if (trim($value) != '' &&
                        preg_match('/[\'^£$%&*()}{@#~?><>, |=_+¬-]/', $value)
                    ) {
                        $errors[] = __('VAT Number cannot contain space and special characters');
                    } else {
                        $value = preg_replace("/<script.*?\/script>/s", "", $value) ? : $value;
                        $fields[$code] = $value;
                    }
